...
```{r}
cts <- assays(experiments(maex)[["genelstpm"]])[["count"]]
tpms <- assays(experiments(maex)[["gene"]])[["TPM"]]
phn <- pData(maex)
```

Although no statistical testing will be performed in this report, we use the
sample annotation denoted ``r phenoid`` to stratify and color the samples for some
of the plots.
...
if (sum(rowSums(counts(sceset)[grep("^ERCC-", rownames(sceset)), ] > 0) > 
        min(table(pData(sceset)[, phenoid]))) > 1) {
  withcontrols <- TRUE
...
cells are colored by the ``r phenoid`` annotation.

```{r}
plotPhenoData(sceset, aes_string(x = "log10_total_counts", y = "total_features", colour = phenoid))
plotPhenoData(sceset, aes_string(x = "pct_exprs_top_200_features", y = "total_features", colour = phenoid))
if (withcontrols) {
  plotPhenoData(sceset, aes_string(x = "total_features", y = "pct_exprs_feature_controls", color = phenoid))
}
```
...
```{r}
scater::plotPCA(sceset, colour_by = phenoid)
scater::plotTSNE(sceset, colour_by = phenoid)
```
