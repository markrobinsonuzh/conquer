...
set_exprs(sceset, "exprs") <- log2(calculateCPM(sceset) + 1)
```
...
```{r}
sceset <- sceset[, sceset$total_features > 5]
```
...
plotPhenoData(sceset, aes_string(x = "pct_counts_top_200_features", y = "total_features", 
                                 colour = paste(phenoid, collapse = "."))) + 
  guides(colour = guide_legend(nrow = nrw, byrow = TRUE)) + theme(legend.position = lps)
if (withcontrols) {
...
if (!is.null(tpms)) {
  if (any(duplicated(tpm(sceset))) || any(duplicated(t(tpm(sceset))))) {
    tpm(sceset) <- tpm(sceset) + matrix(rnorm(length(tpm(sceset)), mean = 0, sd = 1e-5), 
                                        nrow = nrow(tpm(sceset)))
  }
  tsne <- tryCatch(
    scater::plotTSNE(sceset, exprs_values = "tpm", return_SCESet = TRUE),
    error = function(e) NULL)
} else {
  if (any(duplicated(exprs(sceset))) || any(duplicated(t(exprs(sceset))))) {
    exprs(sceset) <- exprs(sceset) + matrix(rnorm(length(exprs(sceset)), mean = 0, sd = 1e-5), 
                                            nrow = nrow(exprs(sceset)))
  }
  tsne <- tryCatch(
    scater::plotTSNE(sceset, exprs_values = "exprs", return_SCESet = TRUE),
    error = function(e) NULL)
}
if (!is.null(tsne)) {
