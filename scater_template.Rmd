...
phn[, paste(phenoid, collapse = ".")] <- interaction(phn[, phenoid])
```
...
```{r}
scater::plotPCA(sceset, colour_by = paste(phenoid, collapse = "."))
if (length(phenoid) > 1) {
  for (pid in phenoid) {
    scater::plotPCA(sceset, colour_by = pid)
  }
}
scater::plotTSNE(sceset, colour_by = paste(phenoid, collapse = "."))
if (length(phenoid) > 1) {
  for (pid in phenoid) {
    scater::plotTSNE(sceset, colour_by = pid)
  }
}
```
